<?php
/**
 * masthead-search.php — Masthead para página de busca
 * Igual ao masthead-inner: wordmark compacto + nav completa + busca expansível + burger
 */
$nav_items = [
...
?>
<header class="dsi-masthead dsi-masthead--inner dsi-masthead--search-page" role="banner">
    ...
    <nav class="dsi-masthead__nav" aria-label="Menu principal">
        ...
        if ( $dsi_nav_args ) {
            ...
            foreach ( $nav_items as $label => $url ) {
                echo '<li class="dsi-masthead__nav-item"><a href="' . esc_url( $url ) . '" class="dsi-masthead__nav-link">' . esc_html( $label ) . '</a></li>';
            }
            echo '</ul>';
        }
        ?>
    ...
    <!-- Mobile hamburger -->
    <button class="dsi-masthead__burger" aria-label="Abrir menu" aria-expanded="false" aria-controls="dsi-mobile-nav-search">
...
<script>
(function(){
    var btn = document.querySelector('.dsi-masthead--search-page .dsi-masthead__search-toggle');
    var frm = document.querySelector('.dsi-masthead--search-page .dsi-masthead__search-form');
    if (!btn || !frm) return;
